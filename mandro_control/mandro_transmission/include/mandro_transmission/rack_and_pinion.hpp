...
class RackAndPinion : public Transmission
{
public:
    RackAndPinion(const double radius, const double reduction);
   ...
   */
    ...
    double getActuatorRadius() const { return radius_; }
    double getActuatorReduction() const { return reduction_; }

    inline uint32_t RAD2PULSE(double rad) const { return static_cast<uint32_t>(rad * (10000 / (2 * M_PI))); }
...
inline RackAndPinion::RackAndPinion(const double radius, const double reduction)
    : Transmission(),
      radius_(radius),
      reduction_(reduction)
{
    if (0.0 == radius_)
    {
        throw TransmissionInterfaceException("Transmission radius cannot be zero.");
    }
    if (0.0 == reduction_)
    {
...
inline void RackAndPinion::actuatorToJointEffort(const ActuatorData& act_data,
                                                            JointData&    jnt_data)
{
  assert(numActuators() == act_data.effort.size() && numJoints() == jnt_data.effort.size());
  assert(act_data.effort[0] && jnt_data.effort[0]);
  
  *jnt_data.effort[0] = *act_data.effort[0] * reduction_ / radius_ ;
}

inline void RackAndPinion::actuatorToJointVelocity(const ActuatorData& act_data,
                                                              JointData&    jnt_data)
{
  ...
  *jnt_data.velocity[0] = *act_data.velocity[0] / reduction_ * radius_;
}

inline void RackAndPinion::actuatorToJointPosition(const ActuatorData& act_data,
                                                              JointData&    jnt_data)
{
  ...
  //*jnt_data.position[0] = ((*act_data.position[0]) / reduction_) / (1000.0/radius_);
  *jnt_data.position[0] = *act_data.position[0] / reduction_ * radius_; 
}

inline void RackAndPinion::jointToActuatorEffort(const JointData&    jnt_data,
                                                            ActuatorData& act_data)
{
  assert(numActuators() == act_data.effort.size() && numJoints() == jnt_data.effort.size());
  assert(act_data.effort[0] && jnt_data.effort[0]);

  *act_data.effort[0] = *jnt_data.effort[0] / reduction_ * radius_;
}

inline void RackAndPinion::jointToActuatorVelocity(const JointData&    jnt_data,
                                                              ActuatorData& act_data)
{
  assert(numActuators() == act_data.velocity.size() && numJoints() == jnt_data.velocity.size());
  assert(act_data.velocity[0] && jnt_data.velocity[0]);

  *act_data.velocity[0] = *jnt_data.velocity[0] * reduction_ / (radius_);
}

inline void RackAndPinion::jointToActuatorPosition(const JointData&    jnt_data,
                                                              ActuatorData& act_data)
{
  assert(numActuators() == act_data.position.size() && numJoints() == jnt_data.position.size());
  assert(act_data.position[0] && jnt_data.position[0]);

  //*act_data.position[0] = ((*jnt_data.position[0]) * reduction_) * (1000.0/radius_) ;
  *act_data.position[0] = *jnt_data.position[0] * reduction_ / (radius_);
}
