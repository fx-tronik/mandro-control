...
// mandro
#include <mandro_hw_interface/estop.h>
...
public:
  ...
  virtual void read() = 0;
  virtual void write(ros::Duration period) = 0;
...
protected:
  ...
  double prev_vel_cmd;
  double prev_pos_cmd;
  MoonsServo::OperationMode mode = MoonsServo::OperationMode::Cyclic_Synchronous_Position;
};

class EtherCATJointControlInterface : public JointControlInterface
{
public:
  EtherCATJointControlInterface(ros::NodeHandle &robot_hw_nh, Controller *manager, int slave_no, ActuatorDataContainer &actuators_data);
...
{
public:
  MimicEtherCATJointControlInterface(ros::NodeHandle &robot_hw_nh, Controller *manager, int slave_no, ActuatorDataContainer &actuators_data, int offset_slave_no);
  ...
  using EtherCATJointControlInterface::estop;
private:
  double &offset;
  double prev_offset;
};
...
{
public:
  DummyJointControlInterface(ros::NodeHandle &robot_hw_nh, int slave_no, ActuatorDataContainer &actuators_data);
...
class MandroHardwareInterface : public RobotHW
{
private:
  //Node Handles
  ...
  bool SEstop(mandro_hw_interface::estop::Request &req, mandro_hw_interface::estop::Response &res);
  ...
  typedef std::vector<std::future<int>> EstopFuturesContainer;
  EstopFuturesContainer estops;
...
public:
   ...
   */
  MandroHardwareInterface(ros::NodeHandle &nh, ros::NodeHandle &robot_hw_nh, urdf::Model *urdf_model, std::string ifname, bool in_simulation);
  ~MandroHardwareInterface();
  void init();
