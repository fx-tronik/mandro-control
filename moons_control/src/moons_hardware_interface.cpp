...
using moons_control::hw_interface::MoonsHardwareInterface;

MoonsHardwareInterface::MoonsHardwareInterface(ros::NodeHandle &nh, ros::NodeHandle &robot_hw_nh, urdf::Model *urdf_model, std::string ifname, bool in_simulation)
...
void MoonsHardwareInterface::init()
{
  ...
  if (sim_)
  {
    ...
    for (int i = 1; i <= 7; i++)
    {
      registerControl(new DummyJointControlInterface(i, actuators_data));
    }
  }
  ...
  {
    manager = new Controller(ifname_); // Create EtherCat Manager
    ...
    for (i = 1; i <= n_dof_; i++)
    {
      ...
      {
        registerControl(new MimicEtherCATJointControlInterface(manager, i, actuators_data, 5));
      }
      else
      {
        registerControl(new EtherCATJointControlInterface(manager, i, actuators_data));
      }
    }
  ...
  for (int i = 1; i <= 7; i++)
  {
    ...
    // Read limits from param server
    if (getJointLimits(actuators_data[i].joint_name_, nh_, actuators_data[i].joint_limits))
    {
      if (actuators_data[i].joint_limits.has_velocity_limits)
        ROS_INFO_STREAM_NAMED("moons", "Joint " << actuators_data[i].joint_name_ << " has rosparam velocity limit ["
...
MoonsHardwareInterface::~MoonsHardwareInterface()
{
  shutdown();
}

void MoonsHardwareInterface::shutdown()
{
...
void MoonsHardwareInterface::registerControl(JointControlInterface *control)
{
  controls.push_back(control);
}

void MoonsHardwareInterface::read(const ros::Time time, const ros::Duration period)
{
  for (JointControlInterface *control : controls)
  {
    control->read();
...
void MoonsHardwareInterface::write(const ros::Time time, const ros::Duration period)
{
...
bool MoonsHardwareInterface::checkForConflict(const std::list<ControllerInfo> &info) const
{
  // TODO:checking for conflicts
  return false;
}

void MoonsHardwareInterface::doSwitch(const std::list<hardware_interface::ControllerInfo> &start_list, const std::list<hardware_interface::ControllerInfo> &stop_list)
{
...
inline ros::Time MoonsHardwareInterface::getTime()
{
  return ros::Time::now();
}

inline ros::Duration MoonsHardwareInterface::getPeriod()
{
  return ros::Duration(0.001);
}

void MoonsHardwareInterface::loadURDF(ros::NodeHandle &nh, std::string param_name)
{
